...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            c1 = ch[f]
            dw_channels = make_divisible(min(args[0], max_channels) * width, 8)
            block_lk_size = args[1]
            small_kernal = args[2]
            drop_path = args[3] if len(args) > 3 else 0.3
            args = [c1, dw_channels, block_lk_size, small_kernal, drop_path]
        else:
            c2 = ch[f]
